...
- ~~AI 글 생성 API 호출 비용(Claude API)이 DB 월 비용보다 높을 가능성.~~ **[정정] 당시에는 다른 외부 AI provider를 사용 중이었음. 비용 구조 다름.**
...
| 2   | "Claude API (Sonnet) 사용, 비용 높음"             | **다른 외부 AI provider 사용**                                  | API 비용 구조가 다름, PD의 비용 비교 재검증 필요 |
