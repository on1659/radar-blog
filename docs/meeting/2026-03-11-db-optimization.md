...
- **Umami가 이미 referrer, user agent, 일별/페이지별 조회수를 모두 추적 중 → 자체 Analytics는 Umami의 열화판.**
...
3. Analytics 테이블 제거 → Umami에 위임 (최선), 또는 일별 집계로 전환
...
- AI 글 생성 API 호출 비용(Claude API)이 DB 월 비용보다 높을 가능성.
...
| 2 | **Umami와 자체 Analytics가 중복** — 역할 통합 필요 | 백엔드#2, DBA, 인프라 |
...
회의 결과, 다음 순서로 단계적 구현을 진행하기로 결정:

1. **[즉시] Analytics 테이블 개선**
   - 일별 집계(DailyAnalytics) + Post.viewCount 비정규화로 전환
   - 또는 Umami에 위임하고 자체 Analytics 제거
2. **[이후] ISR 전환**
   - `force-dynamic` 제거, `revalidate` 설정
   - Webhook 후 on-demand revalidation 연결
3. **[이후] 커밋 생성 정책 도입**
   - `chore`/`docs`/`style` 커밋 필터링
   - 일일 배치 생성 검토
