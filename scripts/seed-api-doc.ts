...
레이더.dev는 REST API를 통해 외부에서 글을 발행할 수 있다. GitHub Webhook으로 커밋 기반 자동 발행도 되지만, 직접 API를 호출해서 원하는 글을 올리는 것도 가능하다.
...
async function main() {
  const post = await prisma.post.create({
    data: {
      title: "레이더.dev 블로그 API 사용법 — 외부에서 글 발행하기",
      ...
      readingTime: 4,
      excerpt:
        "레이더.dev 블로그에 외부에서 글을 발행하는 API 사용법. Bearer token 인증, 요청/응답 형식, curl 예시까지.",
      content,
    },
  });
